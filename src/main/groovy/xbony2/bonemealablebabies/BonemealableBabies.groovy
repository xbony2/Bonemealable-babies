...
class BonemealableBabies {
	...
	static class BoneMealHandler {
		...
		void boneMealAnimals(EntityInteractSpecific event){
			if(event.getSide() == Side.SERVER && event.getItemStack()?.getItem() == Items.DYE && event.getItemStack().getItemDamage() == 15 && event?.getTarget() instanceof EntityAgeable && ((EntityAgeable)event.getTarget()).getGrowingAge() < 0){
